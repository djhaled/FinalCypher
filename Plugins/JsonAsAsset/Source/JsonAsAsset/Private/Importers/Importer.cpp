...
void IImporter::LoadObject(const TSharedPtr<FJsonObject>* PackageIndex, TObjectPtr<T>& Object) {
	...
	Path = Path.Replace(TEXT("ShooterGame/Content"), TEXT("/Game"));
...
template <typename T>
void IImporter::LoadObject(FString ObjPath, FString Type, TObjectPtr<T>& Object)
{
	ObjPath = ObjPath.Replace(TEXT("ShooterGame/Content"), TEXT("/Game"));
	//UObject* Test;
	int32 LastDotIndex;
	if (ObjPath.FindLastChar('.', LastDotIndex))
	{
		ObjPath = ObjPath.LeftChop(ObjPath.Len() - LastDotIndex);
	}
	Type = Type.Replace(TEXT("_C"), TEXT(""));
	DownloadWrapper(Object, "WidgetBlueprintGeneratedClass", Type, ObjPath);
}
	...
	for (const TSharedPtr<FJsonValue>& ExportPtr : Exports) {
		...
		if (CanImport(Type) || bDataAsset) {
			...
			if (Type == "AnimSequence" || Type == "AnimMontage") 
				...
				else if (Type == "WidgetBlueprintGeneratedClass") Importer = new UWidgetImporter(Name, File, DataObject, LocalPackage, LocalOutermostPkg, Exports);
