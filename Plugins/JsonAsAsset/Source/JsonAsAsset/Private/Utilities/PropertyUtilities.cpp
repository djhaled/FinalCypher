	...
	if (MapProperty) {
		...
		*static_cast<FName*>(Value) = *NameString;
		...
		if (!SerializedValue.IsEmpty()) {
			...
			//L"CultureInvariantString"
			FString TextObject;
			if (!Object->TryGetStringField("SourceString", TextObject)) { 
				Object->TryGetStringField("CultureInvariantString", TextObject);
			}
			TextProperty->SetPropertyValue(Value, FText::FromString(TextObject));
		}
	}
